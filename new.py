...
SUMMARY_REPORT_FILE = 'summary_report.json'  # File for summary stats
...
async def append_csv(data: list[dict], filename: str):
    """Asynchronously appends a list of dictionaries to a CSV file."""
    if not data:
        return
...
async def process_library_website(session: ClientSession, library_data: dict, semaphore: asyncio.Semaphore):
    ...
    if not base_url or base_url == 'Website Not Found' or not base_url.startswith('http'):
        ...
        return result_data, 0  # Return original data with "Not Found" and score 0
...
async def main():
        ...
        for future in tqdm(asyncio.as_completed(tasks), total=len(tasks), desc="Finding links", unit="website"):
            try:
                ...
                if result:  # process_library_website should always return the dict
                    ...
                    if result['SuggestionLinkGuess'] != "Not Found":
    ...
    summary = {
        'Total Websites Processed': stats['total_processed'],
        'Successful Links Found': stats['successful_links'],
        'Failed Links': stats['failed_links'],
        'Average Score': round(sum(stats['scores']) / len(stats['scores']), 2) if stats['scores'] else 0,
        'Score Distribution': {score: stats['scores'].count(score) for score in set(stats['scores'])}
    }
    await save_json(summary, SUMMARY_REPORT_FILE)
    logging.info(f"Summary Report: {json.dumps(summary, indent=2)}")
